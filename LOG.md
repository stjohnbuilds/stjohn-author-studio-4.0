...
## 25 September 2026 — older reader-plan reconciliation

The retired 2.0 plan targeted a monolithic sample-data preview. Current 4.0 uses the app route/components and actual local stores; the old preview file and sampleProjects path are not current implementation targets. ProofingReader already calls the shared ChapterReader body renderer and word lookup, while retaining its audio/flag wrapper; Quill uses the ChapterReader component. Corrected the stale TODO claim that Proof had an entirely separate word renderer. Signed-in desktop/phone flows and target-tool export acceptance remain open; no runtime, saved data or release changed.
