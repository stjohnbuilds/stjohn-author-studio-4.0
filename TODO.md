...
- [ ] **Publish v4.0.31 to GitHub Releases + fresh Windows build.**
      ...
      is also stale (2026-06-09). the user decides when; then
...
- [x] **"Scan for ACX" tool on the proofer book screen** (top action bar,
      next to See errors — moved OUT of Settings per the user). Measures each
      ...
      pass/fail. Mirrors the tool the user trusts.
...
- [x] **ACX limits (triple-checked via a verification workflow):** RMS
      ...
      practice (the user's -1.5dB / 0.96s files passed ACX). Noise floor
      ...
      163/163 tests pass (incl. live-ffmpeg + the user's real file); renderer
...
- [ ] **the user hands-test (installed v4.0.19):** open a book, click "Scan for
...
- [ ] **Add `electron-updater` so the installed app pulls future updates
      automatically from GitHub Releases.** the user's pain point: every
      ...
      Caveats to call out in the user's voice once done:
...
- [ ] **The NSIS uninstaller exits cleanly but leaves files in
      `C:\Program Files\StJohn Author Studio\` that the user can't delete
...
- [ ] **Stop the "StJohn Author Studio cannot be closed" prompt during
      Windows install** when the app is clearly NOT running. the user hit
...
- [x] **Shared `app/lib/characterPalette.js` is now the single source of
      ...
      stay gray until the user clicks the colour swatch to change them
...
- [x] **Walker now recognises `<p>Vex</p>` as a scene boundary.** the user's
      ...
      (`tests/character-marker.test.mjs`); 140/140 total pass. the user
...
- [x] **CSV parser rewritten position-based.** Column names ignored
      completely (the user 2026-06-04: "even if a column were named
...
- [x] **"Check errors" popup — saved flags OR uploaded CSV, same UI.**
      ...
      Say) AND the user's engineer-template spreadsheet variant (column
...
- [x] **Per-narrator speed memory — shared across Proof / Quill /
      ...
      `'default'`. So the user's Mark chapters open at her last Mark
...
- [x] **Audiobook breakdown: harden narrator lookup.** In
      ...
      scene headings — the user's current manuscript), also try
...
- [x] **Audiobook breakdown: per-chapter character picker.**
      [SUPERSEDED + REVERTED 2026-06-04 — the user said no new UI / no new
...
- [x] **Proof flag default ts: prefer whisper-aligned time when a
      ...
      — completed 2026-06-04
- [ ] **Page-number consistency — BENCHED at the user's request
      2026-06-04.** The whole stack is fragile (PDF page text search +
      ± page adjustment + paging extraction). the user has explicitly
      parked it. Do NOT propose page-number work until she re-opens
      this.

### 🆕 2026-06-01 — Phone companion: three fixes the user reported

All live in `app/phone/page.js` (phone is one big file). Doing them one
at a time, **folder first** (the user's pick). the user uses both iPhone and
Android.
...
- [ ] **Phone: attribute the flag narrator to the tapped word.** Desktop
      ...
      keeps H2 headings; it strips highlight colours). Spec from the user
...
`REVIEW_PROMPT_phone-fixes.md`. NOT archived — needs the user's real-phone
test (rule 7: done only after the user clicks it on a real file).

- [ ] **Phone: folder pick didn't auto-attach each chapter's audio (still
      said "pick").** Root cause (the user 2026-06-01): `mapPulledProofProjects`
...
trace). Fixes applied from it:
  • `audioLibrary.js` pickAudioFile loose match: was substring, so
    ...
    wrong file. (the user's "RIGHT audio" concern.)
...
(0 added audio inputs / <audio>). Still needs the user's real-phone test.
...
coloured manuscript to confirm the user's books); whole-token matcher may
...
- [ ] **Port snapshot system to Typing and Tomes.** Open a fresh
      development assistant session in that project, ask for "port the Drive snapshot
...
- [ ] **Port snapshot system to Script and Sync 3.0.** Same as above
      in that project.
- [ ] **Add Prep to cloud as `prep_projects` table (the user requested
...
- [ ] **Phone Quill edit/delete annotations (the user requested
...
2. Phone ↔ desktop flag round-trip — the user has used it informally; no
...
- [x] **Proof + Quill transcription sync real-file check.** the user
...
- [x] **Phone Quill Back navigation.** the user re-tested the Quill phone
      Back fix and said it seems fixed. — completed 2026-05-27
- [x] **Phone → desktop Quill annotation round-trip.** the user added an
...
- [ ] **Quill home load-speed real check.** The code now has the light
      project-list index. the user should test switching into Quill on her
...
Progress 2026-05-27: the user re-tested the packaged Mac app after
...
Progress 2026-05-27: the user confirmed Proof transcription sync looks
...
Needs packaged-app timing check on the user's large project before this can
be marked complete.

Progress 2026-05-27 overnight: Mac app was rebuilt with the lazy Quill
list path still passing build/package checks. Still needs the user's timing
...
Audio File in the Quill CSV export. Needs the user's real-phone test after
deploy.
...
after Back set the chapter to null, causing a flicker and trapping the user
in the reader. Deployed the fix to production.

Progress 2026-05-27: the user confirmed transcription ports to phone, but
...
reader no-audio state now tells the user to return to the chapter list for
...
Progress 2026-05-27: the user confirmed phone-to-desktop data looked good
...
Progress 2026-05-27: the user tested the Quill phone Back fix and confirmed
...
- Fails on the user's books (returns wrong pages — e.g. p.16 came back as p.4)
...
- Doesn't match the user's mental model: PDF is for page numbers; words live in the manuscript.

**The fix the user signed off on.** Replace the heavy `pdfPaging.pages`
...
8. **Acceptance test (the user's hands):** open Anarchy. Tap a word she
...
Progress 2026-05-27 overnight: the user said page numbers were already
...
- [x] **Tombstone permanent-ghost: `clearTombstone` is exported but
      never called.** If the user deletes a project and re-imports/re-creates
...
- [x] **Audio-extension list drift.** RULES.md lists 6 extensions;
      `audio-guard.js:32` regex covers 8 (adds `.ogg`, `.aac`).
      Reconcile to one source of truth. Low risk for the user.
...
- [x] **Desktop has no per-flag offline queue.** Only the full-book
      debounced push. If the user saves a flag and closes the laptop
...
Part A is hands-on for the user (every mode, real books, real audio).
Part B is the deep dive for development assistant (cloud round-trip, edge cases,
...
### OLD — the user's testing checklist (superseded by FINAL-ROUND-checklist.md)

- [x] **Sign in** — completed 2026-05-25. Supabase project
      `evcusovtjfypfyfvnooy` wired; the user signs in with her existing
      account.
...
- [ ] **Test the InDesign export.** Book detail → Export CSV +
      ...
      highlights underlined? the user is the only one who can verify
      ...
      This is a safe first test, not a replacement for the user's real
      layout check.
...
### Next up after the user's checklist
...
- [x] **Deploy phone to Vercel.** Live at
      **https://stjohn-author-studio-4.vercel.app/phone**. Linked to the
      Vercel project `the user-mackays-projects/stjohn-author-studio-4`,
...
- [ ] **Audio sync in Quill desktop reader.** Optional. Quill works
      without audio. If the user wants audio while annotating on the
...
- [ ] **`local-tooling/hooks/` health check.** Verify every hook handles
      ...
      `context-check` → `no-mess`. Add a `local-tooling/hooks/_test.sh`
...
- [ ] the user runs every minimum-release check on her actual books +
...
- [x] **Proofer highlights now match the imported doc's colours.** —
      completed 2026-07-08. the user: "keep the highlight colours the
...
- [x] **Hardened the preview player after the user's live test.** —
...
- [ ] **Transcription queue: individual transcribes error + freeze.**
      the user (same session): chapter 13 "Failed invoking remote
...
- [x] **Mini preview player under Download clip.** — completed
      2026-07-08. the user's design: play button + slider bar with the
...
- [x] **Merged the two conflicting proofer popups into one.** —
      completed 2026-07-08. the user's feedback on v4.0.27: the new
...
- [ ] **Split arrows do nothing when split is off.** the user (same
...
- [ ] **Transcriptions sometimes vanish.** the user (same session):
...
- [x] **Download clip from a long text selection.** — completed
      2026-07-08 (v4.0.27 ✂️). the user selects 2+ words (a sentence, a
      ...
      the user's data). Also that session: dev `.next` cache corruption
...
- [x] **Proof flags now resolve page numbers by sentence, not just word
      count.** — completed 2026-06-01. the user's chapter 51 was showing page
...
- [x] **"Organise…" button next to "Import…" in Duet's Bulk audio card.** —
      completed 2026-06-01. the user ships a folder of raw chapter audio to
...
- [x] **Every download in the desktop app is now collision-proof.** —
      completed 2026-05-29 (code; pending the user hands-test in the rebuilt
      app). the user has asked many times for this: when you save/export a
...
- [x] **Quill Word doc export restored + bundle exports everything.** —
      ...
      InDesign)" builds a single .zip containing all three (the user's
      ...
      HTTP 200. Still needs the user to click it on a real book to fully
      confirm.

### 2026-05-26 evening — the user's crash-out triage + Proof one-screen rebuild

- [x] **Page-shift explainer text uses the user's exact wording.** Replaced
...
- [x] **The duplicate "Split chapters on sub-headings" panel is gone**
      from both `ImportFlow.js` AND `ManuscriptSetup.js`. the user asked
...
- [x] **PDF wording explains both sources.** Now says "either downloaded
      from Google Docs as PDF, or saved from Word as PDF." the user asked
...
- [x] **Cloud safety audit brief written** at `docs/CLOUD_SAFETY_AUDIT.md`.
      ...
      the cloud-sync package end-to-end. the user handed it off and got
...
- [x] **Hard red warning when page lookup truly fails.** When BOTH the
      ...
      the user's rule: never silently show a guess. — completed 2026-05-26

### 2026-05-25 overnight session 2 — phone v1 functionality parity

the user's ask after seeing the deployed phone: "It's 80% there. Pull
...
- [x] **IndexedDB project cache + reader-location memory** — phone
      ...
      the user left off. Cache is no-overwrite-with-empty (so a
...
- [x] **PinnedTabPanel shared component + tab-stability hook rule.**
      ...
      `local-tooling/hooks/build-checker.sh` with a soft warn (Rule 6) when
...
- [x] **Shared cloud-sync package** —
      ...
      (strips audio paths before any upload — RULES.md emphasizes
...
- [x] **One dialogue warning rule** — engine used to emit 7 issue
      ...
      follow-up quote within ~3 paragraphs. the user can't see the noise
...
- [x] **Edit chapters cog** — gear button next to Chapters header.
      ...
      remove it. Chapter numbers re-flow automatically. the user can fix
      "I accidentally left a chapter in" without re-importing. —
      completed 2026-05-24

- [x] **Auto-assign on character add (in reader)** — when a dialogue
      is selected and the user adds a new character via the chip, the
...
- [x] **Pastel palette + Prep is yellow** — `MODE_TOKENS` now has
      ...
      `CHARACTER_PALETTE` in the user's preferred order. — completed
      2026-05-24
...
- [x] **Fix-quote replays into the exported .docx** — paragraph edits
      ...
      close-quote the user typed in actually shows up in the downloaded
...
- [x] Set up `local-tooling/` with scope-locked hooks per bible Step 2.5 —
      completed 2026-05-23
- [x] Write `RULES.md` — completed 2026-05-23
