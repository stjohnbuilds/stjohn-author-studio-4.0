...
- Auto-update: the installed app checks `latest.yml` / `latest-mac.yml` on the newest published, non-draft GitHub Release in `stjohnbuilds/stjohn-author-studio-downloads`. Packaging and release-copying, above, never publish that release themselves — publishing to GitHub Releases is a separate step, and only after it happens can already-installed copies see the new version.
