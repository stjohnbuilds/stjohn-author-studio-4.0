...
function extractContextParagraphs(sectionHtml, quote) {
  ...
  if (q) {
    ...
    if (idx < 0) {
      ...
      // 2) N-gram overlap score. the user 2026-06-09 v4.0.12: dropped
      ...
      //    "paragraph 0 fallback" symptom the user saw on every flag
      //    after the first.
      if (words.length >= 3) {
...
function LiveTargetParagraph({ target, quote, paragraphStartWordIdx, currentChapterMsIdx, audioPlaying, tone = 'target' }) {
  ...
  if (q) {
    ...
    if (quoteHeadInNorm < 0) {
      ...
      // actual match get painted yellow (the bug the user nearly hit).
...
export default function CheckErrorsDialog({ open, onClose, book, audioUrls }) {
  ...
  useEffect(() => {
    ...
    // the user 2026-06-09: removed the auto-play. The dialog still
    // seeks the player to the flag's timestamp so the play button
    // starts in the right place, but doesn't kick off playback
    // automatically — the user was finding the auto-play disruptive
        ...
        {total === 0 ? (
          ...
          <>
            ...
            <div style={{ marginBottom: 14 }}>
              {/* the user 2026-06-09 v4.0.11: word-following highlight
