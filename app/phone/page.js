...
function ScriptPhoneService({ session, onSignOut, onBackToServices, readerSettings, onOpenSettings, audioFilesByBook, setAudioFilesByBook, audioSectionOverride, setAudioSectionOverride }) {
  ...
  // waiting to reach the cloud. Surfaces as a persistent banner so the user
...
function ScriptChapterView({ book, chapter, section, readerSettings, onBack, onSwitchSection, onOpenSettings, onSaveBook, onSaveFlag, onRemoveFlag, presetAudioFile = null, onManualPickAudio = null }) {
  ...
  const [audioTime, setAudioTime] = useState(0);
  const [syncEnabled, setSyncEnabled] = useState(false);
  // True when the user has tapped "＋ New" to type a narrator not in the list.
  ...
  useEffect(() => {
    ...
    setFlagDraft((prev) => ({
      ...
      // to cover MORE sentences extends the quote — until the user edits it.
  ...
  async function handlePickFolder() {
    if (supportsDirectoryPicker()) {
      try {
        ...
        // No audio in there — let the user try the manual pickers instead.
      } catch (err) {
        setReloadState('idle');
  ...
  return (
    ...
    >
      <input
        ...
        // input falls back to multi-file selection — the user can multi-select
