...
- [x] Archive `stjohnbuilds/stjohn-author-studio-2.0` on GitHub —
      completed 2026-05-23
- [ ] Confirm with the user which other GitHub repos to archive (likely
...
- [x] Home-screen 4-mode segmented switcher added
      ...
      2026-05-23
- [ ] the user opens the new 4.0 app, sees the 4-mode switcher, clicks
...
- [ ] Migrate (optional): point Save Folder at the old `Script and Sync`
      data location if the user wants her existing books in 4.0.
...
- [ ] **Studio landing page (4-mode picker)** — first screen on launch
      ...
      dropping the user straight into Proof. the user flagged this; lives
...
- [ ] **Migrate Proof's `BookSetup` to use `ImportFlow`** — Prep + Duet
      ...
      piece and add the Proof-only steps as panels around it. the user's
...
- [ ] **Quote-insert fixes should patch the original .docx export too**
      ...
      change. But if the user actually adds new text (not just a close
...
- [ ] Mark every Proof Listen button as `verified live` in
      `WIRING_MATRIX.md` after the user clicks it on her real audiobook.
...
- [ ] Port annotation list UI (the + and edit icons the user liked) from
...
- [x] **Strip dialogue warnings down to one rule** — engine used to emit
      ...
      close, missing close, uneven, quote-in-heading). the user said too
