...
export default function ProofingReader({ section, audioUrl, narratorColors, manuscriptPaging = null, pdfPaging = null, pdfPageMap = null, pageNumberAdjustment = 0, includeChapterPreroll = true, defaultListeningSpeed = 2, onSaveFlags, onBack, canPrevChapter = false, canNextChapter = false, onPrevChapter, onNextChapter, sceneOptions = [], onJumpToScene = null, usesCustomDragRegion = false }) {
  ...
  function getAutoPageNumber(wordIdx, quoteText){
    ...
    // when this reader opens. Lets the user nudge ±N pages to match her
    // Word doc when LibreOffice rendering drifts by a hair.
    const adjustment = Number(pageNumberAdjustment) || 0;

    if (pdfMatch?.pageNumber) {
      // the user 2026-05-26 bug fix: when the PDF lookup found a page whose
    ...
    // the user 2026-05-26: PDF quote-search is a fragile shortcut; the
    ...
    // No exact map available anywhere — flag it. the user's rule: never guess.
    return '?';
  }
