...
export default function Home() {
  ...
  return (
    <div style={{ ...modeAccentVars('proof'), minHeight:'100vh', background:'var(--cream)' }}>
      ...
      {view==='reading'    && activeSection && activeBook && <ProofingReader section={activeSection} audioUrl={audioUrl} narratorColors={activeBook.narratorColors} manuscriptPaging={activeBook.manuscriptPaging} pdfPaging={activeBook.pdfPaging} pageNumberAdjustment={Number(activeBook.pageNumberAdjustment) || 0} includeChapterPreroll={readerIncludeChapterPreroll} defaultListeningSpeed={readerDefaultListeningSpeed} onSaveFlags={(cid,sid,f)=>saveFlags(cid,sid,f)} onBack={(snapshot)=>{ if(snapshot) setBookPlayerState({ currentTime: Math.max(0, Number(snapshot.currentTime) || 0), isPlaying: !!snapshot.isPlaying, playbackRate: Math.max(0.5, Math.min(3, Number(snapshot.playbackRate) || 1)) }); setBookPlayerLabel(buildPersistentPlayerLabel(activeBook, activeSection)); setView('bookDetail'); restoreBookDetailScroll();}} canPrevChapter={canNavigateReaderChapter(-1)} canNextChapter={canNavigateReaderChapter(1)} onPrevChapter={()=>navigateReaderChapter(-1)} onNextChapter={()=>navigateReaderChapter(1)} sceneOptions={readerSceneOptions} onJumpToScene={jumpToReaderScene} usesCustomDragRegion={usesCustomDragRegion} />}
      {transferNotice && (
