...
export default function Home() {
  ...
  // case so the desktop doesn't push a stale book back over it. (the user
  ...
  useEffect(() => {
    ...
    async function doPull() {
      try {
        ...
        // sometimes sticks" race the user hit.
  ...
  return (
    <div style={{ ...modeAccentVars('proof'), minHeight:'100vh', background:'var(--cream)' }}>
      ...
      />
      {view==='home'       && <HomePage books={books} isElectron={isElectron} dataLocation={dataLocation} onChangeDataLocation={handleChangeDataLocation} onNew={()=>setView('newBook')} onOpen={b=>{setActiveBook(b);setView('bookDetail');}} onImport={importBooks} onExport={exportBackup} onElectronImport={handleImport} authEmail={authSession?.user?.email || ''} onSignOut={handleSignOut} />}
