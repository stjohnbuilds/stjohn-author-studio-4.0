...
### the user's testing checklist (do these in order)
...
- [ ] **Deploy phone to Vercel.** Once the phone is feature-complete
      enough for the user to use, push it live. `vercel.json` may need a
...
**The mandate, in the user's words:** ONE system. ONE BookDetail. ONE reader.
...
- [ ] **A3. Extract `<FlagForm>`** → `app/components/FlagForm.js`.
      the user hasn't asked Quill to flag (Quill annotates), but Proof's
...
- [ ] **E4. Real-file end-to-end test pass** (the user's). Open each
...
**Why this is urgent:** the user has flagged this many times. The four
...
- [ ] **Step 4 — Migrate Proof** to use `<ChapterReader>`. **Highest
      risk — the user's anchor mode that has been working since v3.0.**
...
`BookDetailView` instead of using shared `BookDetail`. If the user wants
Prep merged in too later, ChapterReader gains a `splitUnits` prop.

- [x] **Step 6 — Update docs and RULES.md.** ChapterReader added to
      `docs/SHARED_COMPONENTS.md` as the canonical reader. "What's NOT
      shared yet → Reader" bullet rewritten to scope to just the
      pending Proof migration. RULES.md architecture rule #1 rewritten
...
- [ ] **Step 8 — Real-file end-to-end test pass.** the user opens each
...
### Same-source consolidation pass (the user's "fix once, fixed everywhere")

These came out of the 2026-05-24 session where the user said every mode's
