...
// Build a direct (audioTime → msWordIdx) lookup table from whisper alignment data.
// Uses alignment[i].wordObj.start directly — no intermediate whisper-word index needed.
function buildDirectSyncTable(alignment, minConfidence = 0.35) {
  const rows = [];
  ...
  // Pass 2: Remove outlier jumps.
  // If the words-per-second between two consecutive anchors is wildly different
  // from the overall average, the anchor is likely a false match.
  ...
  const maxWps = Math.max(avgWps * 3, 10); // Allow 3x average or minimum 10 wps
  ...
  for (let i = 1; i < mono.length; i++) {
    ...
    if (dt > 0.05) {
      ...
      if (localWps > maxWps) {
        // This anchor jumps too far forward — skip it
        continue;
      }
    }
    cleaned.push(cur);
  ...
  console.log(`SyncTable: ${rows.length} raw → ${mono.length} monotonic → ${cleaned.length} cleaned (avgWps=${avgWps.toFixed(1)}, maxWps=${maxWps.toFixed(1)})`);
  return cleaned;
}

// Binary search: returns manuscript word index at the given audio time.
function getMsIdxAtTime(syncTable, audioTime, fallbackIdx) {
...
function getAudioTimeForMsIdx(syncTable, targetMsIdx) {
  const tbl = Array.isArray(syncTable) ? syncTable : [];
...
export default function ProofingReader({ section, audioUrl, narratorColors, manuscriptPaging = null, pdfPaging = null, includeChapterPreroll = true, defaultListeningSpeed = 2, onSaveFlags, onBack, canPrevChapter = false, canNextChapter = false, onPrevChapter, onNextChapter, sceneOptions = [], onJumpToScene = null }) {
