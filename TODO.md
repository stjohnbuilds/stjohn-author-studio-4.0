...
- [ ] **Studio landing page + Supabase login** — first screen on
      ...
      dropping straight into Proof like it does now. Match the user's
      pastel aesthetic — no dark/wine colors. A similar login UI
      existed in a previous attempt (look in `Manuscript Prepper 1.0`
      or the archived apps before designing from scratch). the user's
      Supabase project is `evcusovtjfypfyfvnooy`. URL + anon key:
      ask the user or find in any archived `.env` / `supabase.js`. Do
...
- [ ] **Quill & Ink mode** — port from the alpha at
      ...
      Prep. Annotation list with `+` and ✏️ icons (the user liked these
...
- [ ] **Supabase cloud-sync wiring** — single shared package every
      ...
      that strip audio paths before any upload. RULES.md emphasizes
      this three times.
...
- [ ] **Phone companion** — port from
      ...
      attempt the user thinks was solid). Login (Supabase), project list,
